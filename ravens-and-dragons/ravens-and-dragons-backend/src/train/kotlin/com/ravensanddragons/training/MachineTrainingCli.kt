...
fun main(args: Array<String>) {
    ...
    val datasetGenerator = MachineTrainedDatasetGenerator(
        clock = clock,
    ...
    val artifactWriter = MachineTrainedArtifactWriter(objectMapper)
    val artifactReader = MachineTrainedArtifactReader(objectMapper)
    ...
    val modelWithSummary = model.copy(
        trainingSummary = model.trainingSummary.copy(
            selfPlayBotIds = options.selfPlayBotIds,
            gamesPerMatchup = options.gamesPerMatchup,
            sampleStride = options.sampleStride,
            maxSampledPositionsPerGame = options.maxSampledPositionsPerGame,
...
private fun runEvolution(
    ...
    val evolutionLoop = MachineTrainedEvolutionLoop(
        clock = clock,
        progressListener = TrainingProgressListener { completed, total ->
...
private data class TrainingCliOptions(
    ...
    val outputDir: Path = Path.of("build", "machine-trained-candidate"),
    ...
    fun resolvedRunId(now: Instant): String {
        ...
        require(runIdPattern.matches(value)) {
            "Machine training run id may contain only letters, numbers, dots, underscores, and hyphens."
        }
        return value
    }
    ...
    ): MachineTrainedArtifactRunSummary =
        MachineTrainedArtifactRunSummary(
            runId = runId,
            mode = mode.name,
    ...
    companion object {
        ...
        fun parse(args: List<String>): TrainingCliOptions {
            ...
            return TrainingCliOptions(
                ...
                outputDir = values.lastValue("output-dir")?.let(Path::of) ?: Path.of("build", "machine-trained-candidate"),
...
private fun MachineTrainedModel.withEvolutionSummary(
    options: TrainingCliOptions,
    runId: String,
    artifactPath: Path,
    reportPath: Path,
...
private fun MachineTrainingSummary.copyForEvolution(
    options: TrainingCliOptions,
    runId: String,
    artifactPath: Path,
    reportPath: Path,
