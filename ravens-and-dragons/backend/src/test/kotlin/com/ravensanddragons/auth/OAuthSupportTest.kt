...
class OAuthSupportTest {
    ...
    fun `oauth authorization redirect uses forwarded https headers for base url`() {
        val result = mockMvc.get("/oauth2/authorization/google") {
            secure = false
            header("Host", "ravens-and-dragons.railway.internal")
            header("X-Forwarded-Proto", "https")
            header("X-Forwarded-Host", "ravens-and-dragons-production.up.railway.app")
        ...
        kotlin.test.assertTrue(
            redirectUrl.contains("redirect_uri=https://ravens-and-dragons-production.up.railway.app/login/oauth2/code/google"),
