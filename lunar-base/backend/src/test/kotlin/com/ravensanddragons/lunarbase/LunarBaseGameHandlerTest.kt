...
class LunarBaseGameHandlerTest {
    ...
    private val fakeCatalog = FakeLunarBaseRuntimeCardCatalog()
    private val handler = LunarBaseGameHandler(
        objectMapper = objectMapper,
        clock = Clock.fixed(Instant.parse("2026-06-01T00:00:00Z"), ZoneOffset.UTC),
        cardCatalog = LunarBaseCompositeRuntimeCardCatalog(fakeCatalog, LunarBaseStandardRuntimeCardCatalog)
    )
    ...
    fun createsInitialDealWithPrivateHands() {
        ...
        assertEquals(false, rawStationCard.has("type"))
        assertEquals(false, rawStationCard.has("cardCost"))
        ...
        assertEquals(setOf("id", "name"), game.privateState.at("/hands/0/0").fieldNames().asSequence().toSet())
    ...
    fun commandResponseIncludesOnlyActingUsersViewerDataAfterClaimingSeat() {
        ...
        val result = handler.applyCommandResult(
            created,
            command("claimSeat", 1).put("seatIndex", 1).put("playerUserId", "user-2").put("displayName", "Ben"),
            "user-2"
        )
        val claimed = result.state

        val response = handler.commandResponse(result, claimed, "user-2")
        val game = response.get("game")
        val viewer = game.get("viewer")

        assertEquals(1, viewer.get("seatIndex").asInt())
        assertEquals(3, viewer.get("hand").size())
        assertEquals(3, game.get("players").get(0).get("handCount").asInt())
        assertEquals(3, game.get("players").get(1).get("handCount").asInt())
        assertEquals(false, game.get("players").get(0).has("hand"))
        assertEquals(false, game.get("players").get(1).has("hand"))
        assertEquals(false, game.has("message"))
        assertEquals("Ben joined.", response.get("message").asText())
        assertEquals(false, claimed.publicState.has("message"))
    }
    ...
    fun choosingMainActionResolvesThroughActorInputAndAdvancesTurn() {
        ...
        assertEquals("Terran Outpost", chooseState.sourceCardName)
        assertEquals(listOf("Draft 1 card", "Build 2 modules; Draw 1 card"), chooseState.interaction?.buttons?.map { it.label })
        assertEquals(false, choosing.publicState.at("/actionState/interaction").has("buttons"))
        assertEquals(false, choosing.publicState.at("/actionState/interaction").has("interactionPrompt"))
        assertEquals(true, choosing.publicState.at("/actionState/interaction/actionText").isMissingNode)
        assertEquals("Choose one:\nDraft 1 card\nBuild 2 modules; Draw 1 card", choosing.readClientPublicState().actionState.interaction?.actionText)
    ...
    @Test
    fun flippingStationUpdatesRemainingActionTextAfterEachFlip() {
        val game = seatedGameWithPlayers { _, player -> player }
        val stationId = game.readPublicState().players[0].board[0].card.id
        ...
        assertEquals("Flip 2 stations (1 left)", interactionActionText(firstFlip.readPublicState().actionState.interaction))
    }
    ...
    @Test
    fun drawingStockUpdatesRemainingActionTextAfterEachDraw() {
        var game = handler.createGame("LUNAR01", createRequest(), "creator")
        game = handler.applyCommand(game, command("claimSeat", 1).put("seatIndex", 0).put("playerUserId", "user-1").put("displayName", "Ada"), "user-1")

        val firstDraw = handler.applyCommand(
            actionGame(game, "draw", remaining = 2, mainActionChosen = true),
            command("drawStock", game.readPublicState().version),
            "user-1"
        )

        assertEquals(1, firstDraw.readPublicState().actionState.interaction?.remaining)
        assertEquals("Draw 2 cards (1 left)", interactionActionText(firstDraw.readPublicState().actionState.interaction))
    }
    ...
    fun draftingSupplyRepeatsUntilTheRequestedCardsAreTaken() {
        ...
        assertEquals("Draft 2 cards (1 left)", interactionActionText(firstDraft.readPublicState().actionState.interaction))
    ...
    @Test
    fun draftingAnyInfluenceTriggersEffectsIncludingTheDraftedInfluence() {
        val game = handler.createGame("LUNAR01", createRequest(playerCount = 2, useInfluences = true), "creator")
        val publicState = game.readPublicState()
        val readyGame = game.copy(
            publicState = objectMapper.valueToTree(
        ...
        val drafted = handler.applyCommand(
            actionGame(readyGame, "draft", mainActionChosen = true),
            command("draftSupply", publicState.version).put("slotIndex", 0),
            "user-1"
        )
        val interaction = drafted.readPublicState().actionState.interaction

        assertEquals(null, drafted.readPublicState().supply.singleOrNull())
        assertEquals("space-cowboy", drafted.readPrivateState().hands[0].last().id)
        assertEquals("stealCredits", interaction?.kind)
        assertEquals("Steal 2 credits", interactionActionText(interaction))
        assertEquals("Space Cowboy Capitalism", drafted.readPublicState().actionState.sourceCardName)
    }
    ...
    fun discardingHandCardsRepeatsUntilTheRequestedCardsAreDiscarded() {
        ...
        assertEquals("Discard 2 cards (1 left)", interactionActionText(firstDiscard.readPublicState().actionState.interaction))
    ...
    @Test
    fun discardingThisInfluenceTriggersItsEffect() {
        var game = handler.createGame("LUNAR01", createRequest(playerCount = 2, useInfluences = true), "creator")
        game = handler.applyCommand(game, command("claimSeat", 1).put("seatIndex", 0).put("playerUserId", "user-1").put("displayName", "Ada"), "user-1")
        game = replaceCurrentPlayerHand(
            game,
            listOf(LunarBaseCard(id = "entropic-cascade", type = "influence", name = "Entropic Cascade"))
        )

        val discarded = handler.applyCommand(
            actionGame(game, "discard", mainActionChosen = true),
            command("discardHandCard", game.readPublicState().version).put("cardId", "entropic-cascade"),
            "user-1"
        )
        val interaction = discarded.readPublicState().actionState.interaction

        assertEquals("entropic-cascade", discarded.readPrivateState().discard.first().id)
        assertEquals("draw", interaction?.kind)
        assertEquals("Draw 4 cards (4 left)", interactionActionText(interaction))
        assertEquals("Entropic Cascade", discarded.readPublicState().actionState.sourceCardName)
    }
    ...
    fun discardActionKeepsRequestedAmountUntilHandIsEmpty() {
        ...
        assertEquals("Discard 3 cards (3 left)", interactionActionText(discardingPublicState.actionState.interaction))
    ...
    @Test
    fun buildingDomeOrLaikaMemorialTriggersMatchingEffects() {
        var game = handler.createGame("LUNAR01", createRequest(playerCount = 2, useInfluences = true), "creator")
        val publicState = game.readPublicState()
        game = game.copy(
            publicState = objectMapper.valueToTree(
                publicState.copy(
                    ...
                    supply = listOf(LunarBaseCard(id = "laikas-paradise", type = "influence", name = "Laika's Paradise"))
                )
            )
        )
        game = putCardInCurrentPlayersHand(game, matchingModule("struve-dome").copy(name = "Struve Dome"))
        game = actionGame(game, "build", mainActionChosen = true)

        val played = handler.applyCommand(
            game,
            command("buildModule", game.readPublicState().version)
                .put("cardId", "struve-dome")
                .put("x", 1)
                .put("y", 0)
                .put("rotation", 0),
            "user-1"
        ...
        assertEquals("struve-dome", played.readPublicState().players[0].board.last().card.id)
        assertEquals("draft", interaction?.kind)
        assertEquals("Draft 1 card (1 left)", interactionActionText(interaction))
        assertEquals("Laika's Paradise", played.readPublicState().actionState.sourceCardName)
    }

    @Test
    fun buildingModuleUpdatesRemainingActionTextAfterEachBuild() {
        var game = handler.createGame("LUNAR01", createRequest(), "creator")
        game = handler.applyCommand(game, command("claimSeat", 1).put("seatIndex", 0).put("playerUserId", "user-1").put("displayName", "Ada"), "user-1")
        game = putCardInCurrentPlayersHand(game, matchingModule("module-build-1"))
        game = putCardInCurrentPlayersHand(game, matchingModule("module-build-2"))
        game = actionGame(game, "build", remaining = 2, mainActionChosen = true)

        val played = handler.applyCommand(
            game,
            command("buildModule", game.readPublicState().version)
                .put("cardId", "module-build-1")
                .put("x", 1)
                .put("y", 0)
                .put("rotation", 0),
            "user-1"
        ...
        assertEquals("Build 2 modules (1 left)", interactionActionText(played.readPublicState().actionState.interaction))
    }
    ...
    @Test
    fun scopedEachPlayerActionMakesEachPlayerActInTurnOrder() {
        ...
        val firstInteraction = played.readPublicState().actionState.interaction

        assertEquals("flipStation", firstInteraction?.kind)
        assertEquals(0, firstInteraction?.actorIndex)
        assertEquals("Crazy President", played.readPublicState().actionState.sourceCardName)

        val firstFlipped = handler.applyCommand(
            played,
            command("flipStation", played.readPublicState().version),
            "user-1"
        )

        assertEquals(true, firstFlipped.readPublicState().players[0].board[0].card.flipped)
        assertEquals("flipStation", firstFlipped.readPublicState().actionState.interaction?.kind)
        assertEquals(1, firstFlipped.readPublicState().actionState.interaction?.actorIndex)

        val secondFlipped = handler.applyCommand(
            firstFlipped,
            command("flipStation", firstFlipped.readPublicState().version),
            "user-2"
        )

        assertEquals(true, secondFlipped.readPublicState().players[1].board[0].card.flipped)
        assertEquals(choosingMainActionPhase, secondFlipped.readPublicState().actionState.phase)
        assertEquals(null, secondFlipped.readPublicState().actionState.interaction)
    }

    @Test
    fun chooseOpponentViewsChosenPlayersHandForActorOnly() {
        var game = seatedGameWithPlayers { _, player -> player }
        game = putCardInCurrentPlayersHand(
            game,
            LunarBaseCard(id = "spybot-test", type = "agent", name = "Spybot.py")
        )
        val targetHandIds = game.readPrivateState().hands[1].map { it.id }
        ...
        val chooseOpponent = played.readPublicState().actionState.interaction

        assertEquals("chooseOpponent", chooseOpponent?.kind)
        assertEquals(listOf("Ben"), chooseOpponent?.buttons?.map { it.label })

        val viewing = handler.applyCommand(
            played,
            command("choosePlayer", played.readPublicState().version).put("playerIndex", 1),
            "user-1"
        )
        val interaction = viewing.readPublicState().actionState.interaction
        val actorView = handler.gameView(viewing, "user-1")
        val targetView = handler.gameView(viewing, "user-2")

        assertEquals("viewHand", interaction?.kind)
        assertEquals(0, interaction?.actorIndex)
        assertEquals(1, interaction?.targetPlayerIndex)
        assertEquals(listOf("Done viewing"), interaction?.buttons?.map { it.label })
        assertEquals(targetHandIds, actorView.get("viewer").get("revealedHands").get(1).map { it.get("id").asText() })
        assertEquals(false, targetView.get("viewer").has("revealedHands"))
    }
    ...
    fun chosenPlayerScopeUsesTheOpponentChosenEarlier() {
        ...
        val discarding = handler.applyCommand(
            game,
            command("chooseActionOption", game.readPublicState().version).put("optionIndex", 1),
            "user-1"
        )
        val cardId = discarding.readPrivateState().hands[1].first().id

        assertEquals("discard", discarding.readPublicState().actionState.interaction?.kind)
        assertEquals(1, discarding.readPublicState().actionState.interaction?.actorIndex)

        val discarded = handler.applyCommand(
            discarding,
            command("discardHandCard", discarding.readPublicState().version).put("cardId", cardId),
            "user-2"
        )

        assertEquals(cardId, discarded.readPrivateState().discard.first().id)
        assertEquals(choosingMainActionPhase, discarded.readPublicState().actionState.phase)
    }

    @Test
    fun eachOpponentScopeStartsAfterCurrentActorInTurnOrder() {
        val game = seatedGameWithPlayers(playerCount = 3) { _, player -> player }
        val action = LunarBaseActionNode(
            kind = "scoped",
            scope = "EACH_OPPONENT",
            actions = listOf(LunarBaseActionNode(kind = "gainCredits", amount = 1, amountKind = "literal"))
        )
        val publicState = game.readPublicState().copy(
            actionState = LunarBaseActionState(
                phase = resolvingActionPhase,
                stack = listOf(LunarBaseActionFrame(1, action))
            )
        )

        val (resolved, _) = LunarBaseActionEngine("LUNAR01", publicState.version)
            .resolve(LunarBaseMutableGame(publicState, game.readPrivateState()))

        assertEquals(4, resolved.players[0].credits)
        assertEquals(3, resolved.players[1].credits)
        assertEquals(4, resolved.players[2].credits)
    }

    @Test
    fun neighborsOfTargetScopeUsesUniqueNeighborsForTwoPlayers() {
        val game = seatedGameWithPlayers { _, player -> player }
        val action = LunarBaseActionNode(
            kind = "scoped",
            scope = "NEIGHBORS_OF_TARGET",
            actions = listOf(LunarBaseActionNode(kind = "gainCredits", amount = 1, amountKind = "literal"))
        )
        val publicState = game.readPublicState().copy(
            actionState = LunarBaseActionState(
        ...
        val choosing = LunarBaseActionEngine("LUNAR01", publicState.version)
            .resolve(LunarBaseMutableGame(publicState, game.readPrivateState()))
            .first
        val chosen = LunarBaseActionEngine("LUNAR01", publicState.version)
            .choosePlayer(LunarBaseMutableGame(choosing, game.readPrivateState()), 0)
            .first

        assertEquals(3, chosen.players[0].credits)
        assertEquals(4, chosen.players[1].credits)
    }
    ...
    @Test
    fun noShuttleCreditsEffectPreventsSupplyRefillCredits() {
        val game = handler.createGame("LUNAR01", createRequest(playerCount = 2, useInfluences = true), "creator")
        val publicState = game.readPublicState()
        val privateState = game.readPrivateState()
        ...
        val refillReadyGame = game.copy(
            publicState = objectMapper.valueToTree(
                publicState.copy(
                    ...
                    supply = listOf(LunarBaseCard(id = "embargo", type = "influence", name = "Terran Embargo")),
                    stockCount = stock.size
                )
            ),
            privateState = objectMapper.valueToTree(privateState.copy(stock = stock))
        ...
        assertEquals(3, passed.readPublicState().players[0].credits)
    }

    @Test
    fun redOrbsGainCreditsEffectAddsRedOrbCreditsDuringSupplyRefill() {
        val game = handler.createGame("LUNAR01", createRequest(playerCount = 2, useInfluences = true), "creator")
        val publicState = game.readPublicState()
        val privateState = game.readPrivateState()
            ...
            publicState = objectMapper.valueToTree(
                publicState.copy(
                    ...
                    players = publicState.players.replaceAt(
                        ...
                        publicState.players[0].copy(
                            credits = 3,
                            board = publicState.players[0].board + LunarBaseBoardCard(artificialIntellect, 1, 0, 0)
                        )
                    ),
                    supply = emptyList(),
                    stockCount = stock.size
        ...
        assertEquals(2, passed.readClientPublicState().players[0].orbs.red)
        assertEquals(5, passed.readClientPublicState().players[0].credits)
    }

    @Test
    fun forbidDraftOtherInfluenceAllowsOnlyItsOwnInfluenceToBeDrafted() {
        val game = handler.createGame("LUNAR01", createRequest(playerCount = 2, useInfluences = true), "creator")
        val publicState = game.readPublicState()
        val readyGame = game.copy(
            publicState = objectMapper.valueToTree(
                publicState.copy(
                    seats = seatedSeats(),
                    supply = listOf(
                        LunarBaseCard(id = "quarantine", type = "influence", name = "Quarantine"),
                        LunarBaseCard(id = "space-cowboy", type = "influence", name = "Space Cowboy Capitalism"),
                        LunarBaseCard(id = "rover", type = "module", name = "Rover")
                    )
                )
            )
        )
        ...
        val draftedModule = handler.applyCommand(
            blocked,
            command("draftSupply", blocked.readPublicState().version).put("slotIndex", 2),
            "user-1"
        )
        val draftedQuarantine = handler.applyCommand(
            draftedModule,
            command("draftSupply", draftedModule.readPublicState().version).put("slotIndex", 0),
            "user-1"
        )

        assertEquals(listOf("quarantine", "space-cowboy", "rover"), blocked.readPublicState().supply.map { it?.id })
        assertEquals(null, draftedModule.readPublicState().supply[2])
        assertEquals(null, draftedQuarantine.readPublicState().supply[0])
        assertEquals(listOf("rover", "quarantine"), draftedQuarantine.readPrivateState().hands[0].takeLast(2).map { it.id })
    }
    ...
    fun resellingSupplyRepeatsAndInvalidSlotsDoNotGainCredits() {
        ...
        assertEquals("Resell 2 cards (1 left)", interactionActionText(firstResell.readPublicState().actionState.interaction))
    ...
    @Test
    fun forbidStealingCreditsEffectReplacesOpponentChoiceWithSkip() {
        val game = seatedGameWithPlayers { index, player ->
            when (index) {
                0 -> player.copy(credits = 2)
                1 -> player.copy(credits = 5)
        ...
        val publicState = game.readPublicState().copy(
            supply = listOf(LunarBaseCard(id = "lunar-alliance", type = "influence", name = "Lunar Alliance")),
            actionState = LunarBaseActionState(
                phase = resolvingActionPhase,
                mainActionChosen = true,
                stack = listOf(LunarBaseActionFrame(0, LunarBaseActionNode(kind = "stealCredits", amount = 3, amountKind = "literal")))
            )
        )
        ...
        assertEquals("stealCredits", nextPublicState.actionState.interaction?.kind)
        assertEquals("Stealing credits is forbidden", nextPublicState.actionState.interaction?.interactionPrompt?.text)
        assertEquals(listOf(LunarBaseActionButton("Skip stealing credits", "skip")), nextPublicState.actionState.interaction?.buttons)
        assertEquals(2, nextPublicState.players[0].credits)
        assertEquals(5, nextPublicState.players[1].credits)
    }
    ...
    fun stealModuleActionWithNoLegalTargetWaitsForSkip() {
        ...
        assertEquals("No module to steal", nextPublicState.actionState.interaction?.interactionPrompt?.text)
        assertEquals(listOf(LunarBaseActionButton("Skip steal", "skip")), nextPublicState.actionState.interaction?.buttons)
    }

    @Test
    fun actionStateSerializesInteractionPromptWithoutActionText() {
        ...
        assertEquals("No module to steal", actionState.interaction?.interactionPrompt?.text)
        assertEquals("Asteroid Grinder", actionState.activeActions.single().moduleName)
        val serialized = objectMapper.valueToTree<JsonNode>(actionState)
        assertEquals(false, serialized.at("/interaction").has("text"))
    }
    ...
    private fun seatedSeats(): List<LunarBaseSeat> =
        listOf(
            LunarBaseSeat(userId = "user-1", displayName = "Ada"),
            LunarBaseSeat(userId = "user-2", displayName = "Ben")
        )
    ...
    private fun actionGame(
        ...
        val interaction = LunarBaseActionInteraction(
            ...
            remaining = remaining,
            action = action,
            actionText = action.toFullActionText(remaining.takeIf { action.repeatsInInteraction() })
        )
    ...
    private fun interactionActionText(interaction: LunarBaseActionInteraction?): String? {
        val action = interaction?.action ?: return null
        return interaction.actionText ?: action.toFullActionText(interaction.remaining.takeIf { action.repeatsInInteraction() })
    }

    private fun LunarBaseActionNode.repeatsInInteraction(): Boolean =
        when (kind) {
            "build", "discard", "draft", "draw", "resell" -> true
            "flipStation" -> flipAmountKind != "self"
            else -> false
        }

    private fun putCardInCurrentPlayersHand(game: GameRecord, module: LunarBaseCard): GameRecord {
        fakeCatalog.register(module)
        val privateState = game.readPrivateState()
    ...
    private fun seatedGameWithPlayers(
        playerCount: Int = 2,
        transform: (Int, LunarBasePlayerPublic) -> LunarBasePlayerPublic
    ): GameRecord {
    ...
    private class FakeLunarBaseRuntimeCardCatalog : LunarBaseRuntimeCardCatalog {
        private val cardsById = mutableMapOf<String, LunarBaseCard>()

        fun register(cards: Iterable<LunarBaseCard>) {
            cards.forEach(::register)
        }

        fun register(card: LunarBaseCard) {
            cardsById[card.id] = card
        }

        fun register(publicState: LunarBasePublicState) {
            publicState.supply.filterNotNull().forEach(::register)
            publicState.players.flatMap { player -> player.board.map { it.card } }.forEach(::register)
        }

        override fun card(id: String, name: String, flipped: Boolean): LunarBaseCard? =
            cardsById[id]?.copy(id = id, name = name, flipped = flipped)
    }
}
