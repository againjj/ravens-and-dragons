...
data class MachineTrainedDatasetGenerationRequest(
...
class MachineTrainedDatasetGenerator(
    private val selfPlayRunner: MachineTrainedSelfPlayRunner = MachineTrainedSelfPlayRunner(),
    private val clock: Clock = Clock.systemUTC(),
    private val progressListener: TrainingProgressListener = TrainingProgressListener { _, _ -> },
    private val expertStrategyFactory: (MachineTrainedDatasetGenerationRequest, Int) -> GameBotStrategy = { request, seed ->
        BotRegistry(SeededRandomIndexSource(seed))
            .requireSupportedDefinition(request.expertBotId, request.ruleConfigurationId)
            .strategy
    }
) {
    fun generate(request: MachineTrainedDatasetGenerationRequest): MachineTrainedDataset {
        ...
        require(examples.all { it.featureSchemaVersion == MachineTrainedFeatureEncoder.schemaVersion }) {
            "Training dataset generation mixed feature schema versions."
        }

        return MachineTrainedDataset(
    ...
    private fun buildTasks(request: MachineTrainedDatasetGenerationRequest): List<TaskDefinition> {
    ...
    private fun generateTaskResult(
        task: TaskDefinition,
        request: MachineTrainedDatasetGenerationRequest
    ): TaskResult {
            ...
            .forEach { sampled ->
                ...
                sampled.legalMoves.forEach { candidateMove ->
                    ...
                    examples += TrainingExample(
                        ...
                        features = MachineTrainedFeatureEncoder.encode(
                            sampled.snapshot,
                            candidateMove,
                            nextSnapshot
                        ).toList(),
