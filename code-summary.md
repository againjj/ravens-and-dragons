...
- Lunar Base commands claim seats, draw from stock, take supply cards, discard influence cards, play agent cards, play module cards onto player boards, pass turns, and end the game while keeping private hands in viewer-specific game views. Lunar Base public card views expose catalog-derived costs, colonist counts, and achievement ordinals for agents/modules/stations; playing agents/modules spends credits after completed colored and gray orbs reduce the catalog cost, and supply refill grants credits from yellow and gray orbs. Backend public state derives board orb counts, housed colonists, and unique scientific achievements from each player board. The frontend hides animated source/destination cards during pending command animations so hand-to-board module placement does not briefly re-render the source card before the fly animation begins.
