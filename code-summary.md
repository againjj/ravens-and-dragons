...
- Lunar Base commands claim seats, draw from stock, take supply cards, discard agent/influence cards, play module cards onto player boards, pass turns, and end the game while keeping private hands in viewer-specific game views. The frontend hides animated source/destination cards during pending command animations so hand-to-board module placement does not briefly re-render the source card before the fly animation begins.
