...
- Lunar Base commands claim seats, play agents before a main action, choose and resolve main actions through a stack-based action engine, build modules onto boards, draft/resell supply cards, discard hand cards, flip stations, steal credits/modules, view hands, and advance turns after pending end-of-turn supply handling while keeping private hands in viewer-specific game views. Lunar Base rejects duplicate seat occupancy by the same user. Lunar Base public card views expose catalog-derived costs, colonist counts, achievement ordinals, station front/back metadata, and catalog-derived action/effect text; playing agents/modules spends credits after completed colored and gray orbs reduce the catalog cost, and supply refill grants yellow/gray-orb credits. Backend public state derives board orb counts, housed colonists, unique scientific achievements, active action status text, and end-game win/draw labels from persisted game state. The frontend keeps player boards and panels in viewer-relative seat order, shows a fixed action panel for current action status and actor interactions, lets seated viewers reveal their own station's other side locally with flip animation, derives card targetability/dimming from the active action, supports click and drag interactions for hand, supply, discard, board build, and module-steal flows, animates automatic stock draws/station flips, and hides animated source/destination cards during pending command animations.
