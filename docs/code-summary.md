...
- `src/main/kotlin/com/ravensanddragons/game/*.kt`
  ...
  - Machine-learned phase 3 expands `MachineLearnedFeatureEncoder.kt` to schema version 3 with named absolute and relative move-local/resulting-position features for mover geometry, gold geometry, gold distance, raven pressure, mobility, material, immediate replies, repetition risk, and shared evaluation score.
...
- `src/main/resources/bots/machine-learned/*.json`
  - Bundled per-ruleset machine-learned artifacts. The Sherwood artifact for `Michelle` is now schema 3 and matches the absolute/relative feature encoder.
