...
- `platform/src/main/kotlin/com/ravensanddragons/platform/game/GameModuleContract.kt`
  - Platform-owned game module contract and registry validation for assembled game metadata.
- `ravens-and-dragons/ravens-and-dragons-backend/src/main/kotlin/com/ravensanddragons/game/RavensAndDragonsGameModuleDefinition.kt`
  - Ravens and Dragons module metadata for current routes, persistence ownership boundaries, and smoke-check entry points.
...
- `platform/build.gradle.kts` is a Kotlin/JVM library project for shared service code, including Spring web, JDBC, security, OAuth, and generic platform tests.
...
- `app/build.gradle.kts` owns the runnable Spring Boot application, depends on `:platform` and `:ravens-and-dragons:ravens-and-dragons-backend`, assembles the registered game modules, and produces `ravens-and-dragons.jar`.
- Frontend build flow:
...
The platform auth module owns identity and session concerns without moving canonical Ravens and Dragons rules out of the game module.
...
- Deletes session-only guest users on logout or session destruction and calls registered `UserReferenceCleanup` adapters so game modules can release any seats they held without ending games.
- Deletes local accounts transactionally while calling those cleanup adapters to clear nullable game ownership references without deleting games.
...
- Platform tests live under `platform/src/test/kotlin` and cover shared web behavior such as disconnected-client exception normalization.
