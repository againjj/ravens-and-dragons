...
The follow-up bot refactor has now split the old single `GameBots.kt` file into focused bot model, registry, strategy, and shared evaluation modules, extracted bot-turn execution plus grouped-undo handling into `BotTurnRunner.kt`, centralized frontend bot-assignment derivation into one selector model used by both the seat panel and thunk logic, and added direct bot-runner coverage plus a deterministic minimax node-budget regression test. The old `docs/bot-implementation-plan.md` planning document has also been removed, with no remaining in-repo references.
