...
- `src/main/kotlin/com/ravensanddragons/game/*.kt`
  ...
  - Machine-learned phase 3 expands `MachineLearnedFeatureEncoder.kt` to schema version 2 with named move-local features plus after-position features for gold distance, raven pressure, mobility, material, immediate replies, repetition risk, and shared evaluation score.
  - Machine-learned phase 4 now uses `MachineLearnedEvolutionLoop.kt` for population-based Michelle improvement with candidate round-robins, baseline comparisons, survivor selection, mutation, crossover, opening diversity controls, and final promotion-threshold decisions.
...
- `src/main/resources/bots/machine-learned/*.json`
  - Bundled per-ruleset machine-learned artifacts. The Sherwood artifact for `Michelle` is now a schema-2 generated artifact trained from 314 labeled positions across 32 self-play games in the local phase 3 pipeline.
