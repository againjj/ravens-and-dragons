...
Release 2 of the bot plan is now implemented. A fresh supported preset game with exactly one authenticated claimed human seat can assign the opposite open seat to the server-driven `Random` bot for `Original Game`, `Sherwood Rules`, `Square One`, `Sherwood x 9`, or `Square One x 9`; bot seat ids persist separately from human ownership, the seat panel renders assigned bot seats as `Bot: Random`, bot-controlled seats are treated as occupied for later human claim attempts, server-side bot turns run synchronously from canonical Kotlin move generation, and undo is disabled once a bot is present.
...
- `src/main/kotlin/com/dragonsvsravens/game/*.kt`
  ...
  - `GameBots.kt` now owns the bot catalog, release-2 supported-ruleset filtering, random strategy wiring, and canonical legal-move value types used by server-side bot turns.
...
- In a fresh supported preset game, a single claimed human player may also assign `Random` to the opposite open seat for `Original Game`, `Sherwood Rules`, `Square One`, `Sherwood x 9`, or `Square One x 9`, and assigned seats render as `Bot: Random`.
