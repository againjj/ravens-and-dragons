...
The `docs` folder now also includes a Sherwood-focused bot planning document at `docs/bot-implementation-plan.md`, which now locks in first-release decisions, adopts release-two-ready bot-id persistence from the start, and sketches a second release with named `Simon`, `Randall`, and `Maxine` bots plus grouped undo and expanded ruleset support.
...
Bot opponents now support grouped undo and three selectable strategies. A fresh supported preset game with exactly one authenticated claimed human seat can choose `Randall`, `Simon`, or `Maxine` from a seat-panel dropdown and assign it to the opposite open seat for `Original Game`, `Sherwood Rules`, `Square One`, `Sherwood x 9`, or `Square One x 9`; bot seat ids persist separately from human ownership, the seat panel renders assigned bot seats as `Bot: Randall`, `Bot: Simon`, or `Bot: Maxine`, bot-controlled seats are treated as occupied for later human claim attempts, server-side bot turns run synchronously from canonical Kotlin move generation, and undo now reverses the last human move plus the immediate bot reply. `Maxine` now reuses shared Kotlin-only simulation helpers with bounded-depth search plus ruleset-aware evaluation, while `Simon` also reuses that shared evaluation layer instead of keeping a parallel scoring path. If the human move ends the game before a bot reply, or the bot reply ends the game, undo still remains available for that last exchange. The old bot-only undo helper sentence has also been removed from the controls panel, leaving undo availability to speak for itself through the button state.
...
- `src/main/kotlin/com/dragonsvsravens/game/*.kt`
  ...
  - `GameBots.kt` now owns the bot catalog, release-2 supported-ruleset filtering, `Randall`, heuristic `Simon`, and bounded-depth `Maxine` strategy wiring, shared Kotlin-only bot simulation and evaluation helpers, and canonical legal-move value types used by server-side bot turns.
