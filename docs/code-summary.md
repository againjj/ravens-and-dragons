...
- `src/main/kotlin/com/ravensanddragons/game/bot/machine/*.kt`
  ...
  - Machine-trained phase 3 expands `MachineTrainedFeatureEncoder.kt` to schema version 4 with named absolute and relative move-local/resulting-position features for mover geometry, gold geometry, gold distance, raven pressure, mobility, material, immediate replies, opponent capture threats, repetition risk, and shared evaluation score.
...
- `docs/machine-trained-bot-improvements.md`
  - Planning notes for making the evolved `machine-trained` bot `Michelle` stronger, including shallow search, richer fitness, outcome learning, side-specific weights, and evaluation reliability.
- `docs/machine-trained-feature-schema-plan.md`
  - Implementation plan for migrating Michelle to schema 5 with side-specific weight vectors, a compact tactical feature set, structural uncapturability features, artifact/data migration guidance, and rollout tests.
...
- `src/main/resources/bots/machine-trained/*.json`
  - Bundled per-ruleset machine-trained artifacts. The Sherwood artifact for `Michelle` is now schema 4 and matches the absolute/relative feature encoder.
