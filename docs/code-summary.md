...
- `docs/machine-learned-bot.md`
  - Design document for the offline-trained `machine-learned` bot `Michelle`, including runtime architecture, artifact format, completed phase 1, 2, and 3 status, and the next phase 4 strengthening-loop plan.
...
- `src/main/resources/bots/machine-learned/*.json`
  - Bundled per-ruleset machine-learned artifacts. The Sherwood artifact for `Michelle` is now a schema-2 generated artifact trained from the local phase 3 pipeline.
