...
- `src/main/resources/bots/machine-learned/*.json`
  - Bundled per-ruleset machine-learned artifacts. Phase 1 currently ships a placeholder Sherwood artifact for `Michelle`.
...
- Frontend test flow:
  ...
  - `./gradlew botMatchHarnessTest` runs the long bot-vs-bot soak harness separately from the default `test` task.
- Runtime flow:
