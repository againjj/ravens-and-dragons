...
The follow-up bot refactor has now split the old single `GameBots.kt` file into focused bot model, registry, strategy, and shared evaluation modules, extracted bot-turn execution plus grouped-undo handling into `BotTurnRunner.kt`, centralized frontend bot-assignment derivation into one selector model used by both the seat panel and thunk logic, and added direct bot-runner coverage plus a deterministic minimax node-budget regression test. `GameBotsTest.kt` now also carries two disabled manual-only checks for comparing representative depth-2 `MinimaxGameBotStrategy` versus `AlphaBetaGameBotStrategy` behavior and relative timing without slowing or destabilizing the default suite. The old `docs/bot-implementation-plan.md` planning document has also been removed, with no remaining in-repo references. A new design document, `docs/machine-learned-bot.md`, now captures the Sherwood-first architecture and phased implementation plan for `Michelle`, and phase 1 runtime scaffolding is now live: the server can load bundled per-ruleset machine-learned artifacts, expose `Michelle` only for rulesets with a matching artifact, and score legal moves through new backend loader, registry, feature-encoding, scoring, and strategy helpers. Phase 2 is now available locally as well: Gradle exposes a dedicated `train` source set plus `runMachineLearnedTraining`, the offline Kotlin pipeline can run Sherwood self-play, serialize expert-labeled datasets, train Michelle with a per-position ranking update instead of a global positive-minus-negative average, deduplicate repeated `(position, move)` examples, write or reread runtime-compatible JSON through a shared artifact payload contract, parallelize per-game training work across all available CPUs by default, and report coarse decile progress while generating datasets and training models. Phase 3 is now implemented too: the encoder contract is schema version 4 with explicit absolute and relative move-local/resulting-position feature groups, relative feature values are sign-adjusted for raven turns, the bundled Sherwood artifact has been remapped by feature name, and the bot match harness includes a Sherwood-only Michelle-vs-baselines smoke evaluation. Phase 4 now replaces the old single-candidate strengthen mode with a repeatable offline evolution loop: training self-play can still randomize early opening plies, the CLI can evolve Michelle candidates through candidate-only round-robins, survivor selection, mutation, and crossover, then rank the surviving population against the incumbent and configured baselines without feeding those non-candidate games back into selection; evolution mode parallelizes match work across the configured worker count defaulting to all available CPUs, supports multiple repeated seed artifacts, and writes both the best survivor-comparison artifact and every final survivor artifact under the configured output directory. Phase 5 operational hardening is now in place: generated artifacts embed run provenance, training parameters, and evolution promotion summaries; the CLI accepts `--run-id` and defaults generated dataset, artifact, survivor, and report names to a run-id convention; and `docs/machine-learned-training-runbook.md` documents the schema-4 training/evolution/release workflow.
...
- `src/main/kotlin/com/ravensanddragons/game/*.kt`
  ...
  - Bot code is now split across focused files: `BotModels.kt` for shared bot value types, `BotRegistry.kt` for bot ids/names/supported rulesets, dedicated strategy files for `Randall`, `Simon`, `Maxine`, and `Alphie`, `BotStrategySupport.kt` for shared Kotlin-only evaluation and simulation helpers, and `BotTurnRunner.kt` for synchronous bot-turn execution plus grouped human-plus-bot undo handling.
  - Machine-learned phase 1 runtime scaffolding now lives in `MachineLearnedModel.kt`, `MachineLearnedModelLoader.kt`, `MachineLearnedRegistry.kt`, `MachineLearnedFeatureEncoder.kt`, `MachineLearnedMoveScorer.kt`, and `MachineLearnedBotStrategy.kt`, with `BotRegistry.kt` loading `Michelle` from bundled artifacts at startup.
  - Machine-learned phase 2 offline training now lives under `src/train/kotlin/com/ravensanddragons/training`, with dataset generation, self-play, artifact read/write, per-position example keys plus deduplication, a ranking-based trainer, decile progress reporting for the training CLI, and a CLI entrypoint. `MachineLearnedArtifactSupport.kt` now centralizes the shared artifact payload and validation contract used by both runtime loading and offline training.
  - Machine-learned phase 3 expands `MachineLearnedFeatureEncoder.kt` to schema version 4 with named absolute and relative move-local/resulting-position features for mover geometry, gold geometry, gold distance, raven pressure, mobility, material, immediate replies, opponent capture threats, repetition risk, and shared evaluation score.
  - Machine-learned phase 4 now uses `MachineLearnedEvolutionLoop.kt` for population-based Michelle improvement with parallelized candidate-only round-robins, survivor selection, mutation, crossover, opening diversity controls, survivor/incumbent/baseline comparison rankings, progress reporting, and final promotion-threshold decisions.
  - Machine-learned phase 5 operational metadata now extends generated artifacts with optional run provenance, training-parameter summaries, and evolution-promotion summaries while keeping older artifacts loadable.
...
- `docs/machine-learned-bot.md`
  - Design document for the offline-trained `machine-learned` bot `Michelle`, including runtime architecture, artifact format, and completed phase 1 through 5 status.
...
- `src/main/resources/bots/machine-learned/*.json`
  - Bundled per-ruleset machine-learned artifacts. The Sherwood artifact for `Michelle` is now schema 4 and matches the absolute/relative feature encoder.
...
- `src/test/kotlin/com/ravensanddragons/game/MachineLearnedBotPhaseOneTest.kt`
  - Verifies machine-learned artifact validation, Sherwood-only registration, legal move selection, immediate-win preference, and ruleset scoping.
