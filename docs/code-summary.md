...
- Frontend test flow:
  ...
  - `./gradlew test` runs the default Kotlin/Spring test task only.
  - Root `./gradlew check` runs the default Kotlin/Spring test task plus `testFrontend`.
  - Filtered backend runs such as `./gradlew test --tests ...` remain JVM-only and skip the frontend test suite.
...
- The frontend tests still run against the compiled output in `build/generated/frontend-test`, so TypeScript build success remains part of frontend test success.
